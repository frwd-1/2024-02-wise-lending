...
//@audit could this struct be packed more efficiently?
struct MarketState {
    int256 totalPt;
    int256 totalSy;
    int256 totalLp;
...
interface IPendleYt {
    function mintPY(
        address _receiverPT,
        address _receiverYT
    ) external returns (uint256 pyAmount);

    function redeemPY(address _receiver) external returns (uint256);
...
interface IPendleMarket {
    ...
    function activeBalance(address _user) external view returns (uint256);
    ...
    function readState(
        address _router
    ) external view returns (MarketState memory marketState);
...
interface IPendleLock {
    ...
    function positionData(
        address _user
    ) external view returns (LockedPosition memory);
